...
import (
	...
	"github.com/dpopsuev/emcee/internal/infrastructure/cache"
...
func asIssueRepo(r *cache.Repository) repository.IssueRepository { return r }
...
type multiRepo struct {
	stub.StubIssueRepository
	stub.StubFieldRepository
	stub.StubJQLRepository
}
...
func TestCachePassthroughFieldRepository(t *testing.T) {
	...
	fr, ok := wrapped.(repository.FieldRepository)
	if !ok {
...
func TestCachePassthroughJQLRepository(t *testing.T) {
	...
	jr, ok := wrapped.(repository.JQLRepository)
	if !ok {
...
func TestCachePassthroughCommentRepository(t *testing.T) {
	...
	_, ok := wrapped.(repository.CommentRepository)
	if !ok {
