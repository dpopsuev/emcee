...
var _ service.TriageService = (*StubTriageService)(nil)
...
func (s *StubTriageService) GetTriageConfig() service.TriageConfig {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.Config
}

func (s *StubTriageService) SetTriageConfig(cfg service.TriageConfig) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.Config = cfg
}
