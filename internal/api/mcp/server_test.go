...
//nolint:funlen
func newTestServer(t *testing.T) (*sdkmcp.ClientSession, *stub.StubEmceeService) {
	t.Helper()
	svc := &stub.StubEmceeService{}
	...
	svc.Status = &service.HealthStatus{
		Status:   "healthy",
		Backends: []service.BackendHealth{{Name: "test", Configured: true, Status: "healthy"}},
	}
	...
	svc.Config = service.TriageConfig{
		RateLimit: 5,
...
func TestHealthToolDegraded(t *testing.T) {
	...
	svc.Status = &service.HealthStatus{
		Status:   "degraded",
		Backends: []service.BackendHealth{},
...
func TestEmceeTriageConfigSet(t *testing.T) {
	...
	if svc.Config.RateLimit != 20 {
		t.Errorf("rate_limit = %f, want 20", svc.Config.RateLimit)
	}
	if len(svc.Config.AllowList) != 2 {
		t.Errorf("allow_list = %v, want [jira gitlab]", svc.Config.AllowList)
	}
}
