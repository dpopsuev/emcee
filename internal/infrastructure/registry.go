...
type Factory func(name string, backend config.Backend) (repository.IssueRepository, error)
...
func CreateFromConfig(cfg *config.Config) (repos []repository.IssueRepository, warnings []string) {
	mu.Lock()
...
func CreateFromEnv() (repos []repository.IssueRepository, warnings []string) {
	mu.Lock()
	entries := make([]entry, len(registry))
	copy(entries, registry)
	mu.Unlock()
