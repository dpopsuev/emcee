...
// repository.IssueRepository with an LRU+TTL read cache. Write operations
...
var _ repository.IssueRepository = (*Repository)(nil)
...
//nolint:funlen
func New(inner repository.IssueRepository, opts ...Option) *Repository {
	r := &Repository{
		inner:       inner,
	...
	if v, ok := inner.(repository.CommentRepository); ok {
		r.comments = v
		r.caps = append(r.caps, "comments")
	}
	if v, ok := inner.(repository.LaunchRepository); ok {
		r.launches = v
		r.caps = append(r.caps, "launches")
	}
	if v, ok := inner.(repository.FieldRepository); ok {
		r.fields = v
		r.caps = append(r.caps, "fields")
	}
	if v, ok := inner.(repository.JQLRepository); ok {
		r.jql = v
		r.caps = append(r.caps, "jql")
	}
	if v, ok := inner.(repository.DocumentRepository); ok {
		r.docs = v
		r.caps = append(r.caps, "documents")
	}
	if v, ok := inner.(repository.ProjectRepository); ok {
		r.projects = v
		r.caps = append(r.caps, "projects")
	}
	if v, ok := inner.(repository.InitiativeRepository); ok {
		r.initiatives = v
		r.caps = append(r.caps, "initiatives")
	}
	if v, ok := inner.(repository.LabelRepository); ok {
		r.labels = v
		r.caps = append(r.caps, "labels")
	}
	if v, ok := inner.(repository.BulkIssueRepository); ok {
		r.bulk = v
		r.caps = append(r.caps, "bulk")
	}
	if v, ok := inner.(repository.PRRepository); ok {
		r.prs = v
		r.caps = append(r.caps, "prs")
	}
	if v, ok := inner.(repository.PRReviewRepository); ok {
		r.prReviews = v
